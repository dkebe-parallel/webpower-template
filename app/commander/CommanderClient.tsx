...
function PayForm({ onSuccess, slug, domain, businessName, formData }: PayFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch('/api/create-payment-intent', {
      ...
      body: JSON.stringify({
        amount: 49900,
        currency: 'eur',
        metadata: { slug, domain, businessName, email: formData.email },
      }),
    })
...
export default function CommanderClient({ data, slug }: Props) {
  ...
  const fetchIntent = useCallback(async () => {
    ...
    const res = await fetch('/api/create-payment-intent', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ amount: 49900, currency: 'eur', metadata: { slug, domain: selectedDomain, businessName: data.business.name } }),
    })
  ...
  const stripeOptions = clientSecret
    ? { clientSecret, appearance: { theme: 'stripe' as const, variables: { colorPrimary: '#2275FE', borderRadius: '8px', fontFamily: 'Hanken Grotesk, system-ui, sans-serif' } } }
    : undefined
  ...
  const s = {
    ...
    input: { width: '100%', height: '48px', border: '1px solid #E8E8E8', borderRadius: '8px', padding: '0 14px', fontSize: '16px', fontFamily: 'var(--body)', color: '#202020', outline: 'none', boxSizing: 'border-box' } as React.CSSProperties,
    label: { display: 'block', fontSize: '14px', fontWeight: 600, color: '#202020', marginBottom: '6px' } as React.CSSProperties,
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '24px', alignItems: 'start' }}>
            ...
            <div style={s.card}>
              ...
              {data.business.rating && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '14px', color: '#202020' }}>
            ...
            {searched && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                ...
                <DomainRow
                  ...
                  available={isAvailableMock(searchedDomain)}
                  selected={selectedDomain === searchedDomain}
                  onSelect={() => setSelectedDomain(searchedDomain)}
                  primary
                />
                ...
                {alternatives.map(alt => (
                  <DomainRow
                    key={alt}
                    domain={alt}
                    available={isAvailableMock(alt)}
                    selected={selectedDomain === alt}
                    onSelect={() => setSelectedDomain(alt)}
                  />
                ))}
              </div>
            )}
              ...
              <div style={s.card}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '20px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '14px' }}>
                    <span style={{ color: '#999' }}>Site démo</span>
                ...
                <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 20px', display: 'flex', flexDirection: 'column', gap: '8px', paddingTop: '16px', borderTop: '1px solid #E8E8E8' }}>
                  {['Domaine 1 an inclus', 'Hébergement 1 an', 'SEO local', 'Panel admin'].map(item => (
                ...
                <div style={{ borderTop: '1px solid #E8E8E8', paddingTop: '16px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '14px', color: '#999' }}>
                    <span>Sous-total HT</span><span>{(PRICE - TVA).toFixed(2)} €</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '14px', color: '#999' }}>
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flex: 1, minWidth: 0 }}>
        <span style={{ fontWeight: primary ? 700 : 500, fontSize: '15px', color: '#202020', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{domain}</span>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexShrink: 0 }}>
        ...
        {available && (
          <button
            onClick={onSelect}
            style={{
              background: selected ? '#2275FE' : '#fff', color: selected ? '#fff' : '#2275FE',
              border: '1.5px solid #2275FE', borderRadius: '6px', padding: '6px 14px',
